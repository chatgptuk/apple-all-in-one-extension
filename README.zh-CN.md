...
- 每个隐藏邮件地址均可由用户主动读取近期邮件预览，并在本地识别验证码、一键复制。邮件预览只保存在弹窗内存中，不会写入扩展存储。
- 一个安全的行内选择器，统一展示已保存凭据、验证码和可选的隐藏邮件地址创建入口。
- 隔离行内选择器中的智能注册：识别“使用 Apple 登录”、复用已与当前网站关联的有效隐藏地址，或在用户明确操作后一次完成隐藏地址创建和强密码准备。
...
在注册表单中，**智能注册**会在可用时提供两个明确选项：

- **使用 Apple 继续**会激活页面中检测到的“使用 Apple 登录”控件。
- **私密注册**会优先复用已与该网站关联的有效隐藏地址；没有匹配地址时才创建并预留新地址，然后准备一个强密码。如果密码框已经显示，扩展会立即填入；多步骤注册则会在下一步密码框中继续提供同一个密码。

只有当用户在扩展来源的隔离选择器中明确点击相应操作时，扩展才会创建地址或激活网站控件。
...
打开地址详情不会自动读取邮件预览。点击**检查**后才会执行一次有界收件箱扫描；发件人、主题、短预览及本地识别出的验证码只保存在弹窗内存中。关闭弹窗后该列表即被丢弃，持久化的仍只有原有历史“最后收信”时间缓存。
...
- 缓存最近邮件活动。
- 按需读取近期邮件、本地复制验证码，以及直接打开 iCloud 邮件。
...
- 在每个隐藏邮件地址详情中加入按需近期邮件列表，同时不持久化邮件预览。
- 从近期邮件主题/预览中本地识别 4–8 位验证码，并提供一键复制。
- 在隔离行内选择器中加入智能注册，支持识别“使用 Apple 登录”、复用已有地址、明确创建新地址，并让已准备的强密码在多步骤注册流程中于内容脚本内存保留最多十分钟。

### v1.2.15 密码查询可靠性

- 让 Apple 密码查询与智能注册所需的较慢已有地址发现完全分离。
- 区分原生查询错误与真正的空密码库，避免较晚到达的失败覆盖较新的结果。
- 对原生辅助程序偶发返回的空结果重试一次，并兼容不同辅助程序版本的已知响应字段形式。
- 扩展重新加载后，为已经打开的标签页同时修复密码与隐藏邮件地址内容脚本。
- 页面已经自动聚焦登录框时，可信点击现在可以重新打开安全选择器。

### v1.2.16 精确网站密码优先

- 密码结果会先于独立的验证码元数据查询显示。
- 为排队中的元数据查询和交互式密码读取设置上限，避免被忽略的 Touch ID 请求让后续查询一直转圈。
- 优先显示 Apple `sites` 元数据与当前主机名精确匹配的凭据；没有精确匹配时，再按相关域名结果与最近使用顺序显示。

### v1.2.17 聚焦的行内建议

- 当前网站已有 Apple 密码登录项时，只显示已保存登录，不再混入注册类功能。
- 当前网站没有已保存登录时，以“私密注册”作为隐藏地址与强密码的唯一组合入口。
- 从行内选择器移除重复的“隐藏邮件地址”独立入口；仍可通过可编辑输入框的右键菜单创建地址。
- 扩展重载导致运行上下文失效时，旧内容脚本会静默清理界面，不再产生未捕获错误。

### v1.2.18 地址缓存与密码详情

- 在标签页之间切换时复用隐藏邮件地址列表，并支持静默刷新以及跟随创建、停用、删除操作更新缓存。
- 点击已保存登录时既填充当前网页，也在弹窗内展开用户名、遮罩密码、网站及验证码可用状态。
- 敏感信息仅保留在弹窗内存中；验证码过期后停止显示旧码，直到用户明确刷新。

### v1.2.19 每次操作仅触发一次 Touch ID

- 打开密码条目后不再自动继续读取验证码，避免连续出现两次 Touch ID，以及弹窗等待第二次验证时卡住。
- 只有用户在展开的详情卡中明确点击“显示验证码”后，才读取与该账号精确匹配的验证码。

### v1.2.20 验证码填充与详情

- 点击单独的验证码条目后，现在会填充网页并展开用户名、当前验证码、倒计时和网站，不再直接关闭弹窗。
- 展示的验证码复用同一次经 Touch ID 授权的原生读取结果，因此展开详情不会再次触发验证。
- 移除扩展主界面的“锁定密码会话”操作；原生连接结束时仍会自动清理会话和明文缓存。
