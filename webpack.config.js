  ...
  plugins: [
    ...
    new CopyWebpackPlugin({
      patterns: [
        ...
        { from: 'src/styles/apple-design.css', to: path.join(__dirname, 'build', 'src', 'apple-design.css'), force: true },
        { from: 'src/assets/img/icon-source.svg', to: path.join(__dirname, 'build', 'src', 'brand-icon.svg'), force: true },
        {
          from: 'src/icons/symbols.json',
          to: path.join(__dirname, 'build', 'src', 'symbols.js'),
          force: true,
          transform: (content) => Buffer.from(`globalThis.AppleAllInOneSymbols=Object.freeze(${JSON.stringify(JSON.parse(content.toString()))});`),
        },
